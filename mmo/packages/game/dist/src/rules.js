export function currentEnergy(character, now = new Date(), regenMinutes = 10) {
...
export function calculateCultivationReward(baseReward, multiplierBps) {
    return (baseReward * BigInt(multiplierBps)) / 10000n;
}
export function calculateCharacterStats(character, stage, root, equipmentModifiers = []) {
    const bonus = equipmentModifiers.reduce((acc, item) => {
...
export function parseEncounterTable(value) {
    if (!Array.isArray(value))
        return [{ key: "safe-passage", weight: 1 }];
    const rows = value.flatMap((row) => {
        if (!row || typeof row !== "object")
            return [];
        const key = "key" in row ? row.key : undefined;
        const weight = "weight" in row ? row.weight : undefined;
        if (typeof key !== "string" || typeof weight !== "number" || weight <= 0)
            return [];
        return [{ key, weight }];
    });
    return rows.length > 0 ? rows : [{ key: "safe-passage", weight: 1 }];
}
export function simulateCombat(player, enemy, rng, maxTurns = 30) {
    const a = { ...player };
    const b = { ...enemy };
    const log = [];
