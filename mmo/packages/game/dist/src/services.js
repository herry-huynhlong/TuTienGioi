import { Prisma, ActivityStatus, Currency, WalletTxType, ListingStatus } from "@ttg/db";
...
async function mutateWallet(tx, characterId, currency, amount, type, referenceType, referenceId, idempotencyKey) {
    if (amount === 0n)
        throw new GameError("INVALID_AMOUNT", "Số tiền không hợp lệ.");
    ...
    if (balanceAfter < 0n)
        throw new GameError("INSUFFICIENT_FUNDS", "Không đủ Linh Thạch.");
...
export async function creditWallet(db, characterId, currency, amount, type, referenceType, referenceId, idempotencyKey) {
    if (amount <= 0n)
        throw new GameError("INVALID_AMOUNT", "Số tiền không hợp lệ.");
...
export async function debitWallet(db, characterId, currency, amount, type, referenceType, referenceId, idempotencyKey) {
    if (amount <= 0n)
        throw new GameError("INVALID_AMOUNT", "Số tiền không hợp lệ.");
    return isClient(db) ? db.$transaction((tx) => mutateWallet(tx, characterId, currency, -amount, type, referenceType, referenceId, idempotencyKey)) : mutateWallet(db, characterId, currency, -amount, type, referenceType, referenceId, idempotencyKey);
}
export async function startCultivation(db, characterId, minutes, now = new Date()) {
    const allowed = new Set([10, 30, 60, 240, 480]);
    if (!allowed.has(minutes))
        throw new GameError("BAD_DURATION", "Thời gian tu luyện không hợp lệ.");
    ...
    if (active)
        throw new GameError("ACTIVE_ACTIVITY", "Bạn đang có hoạt động tu luyện.");
    const energy = currentEnergy(character, now);
    const cost = Math.max(1, Math.ceil(minutes / 30));
    if (energy < cost)
        throw new GameError("NO_ENERGY", "Không đủ Thể Lực.");
...
export async function claimCultivation(db, characterId, activityId, now = new Date()) {
    return db.$transaction(async (tx) => {
        const job = await tx.cultivationActivity.findUnique({ where: { id: activityId } });
        if (!job || job.characterId !== characterId)
            throw new GameError("NOT_FOUND", "Không tìm thấy hoạt động.");
        if (job.status === ActivityStatus.CLAIMED)
            throw new GameError("ALREADY_CLAIMED", "Phần thưởng đã được nhận.");
        if (job.endsAt > now)
            throw new GameError("NOT_READY", "Hoạt động này chưa hoàn thành.");
        const reward = calculateCultivationReward(job.baseReward, job.multiplierBps);
        await tx.cultivationActivity.update({ where: { id: activityId }, data: { status: ActivityStatus.CLAIMED, claimedAt: now } });
...
export async function attemptBreakthrough(db, characterId, rng = Math.random) {
    return db.$transaction(async (tx) => {
        ...
        if (!next)
            throw new GameError("MAX_REALM", "Bạn đã chạm đến cực hạn hiện tại.");
        if (character.cultivation < next.requiredCultivation)
            throw new GameError("NOT_ENOUGH_CULTIVATION", "Bạn chưa đủ tu vi.");
...
export async function startExploration(db, characterId, minutes, now = new Date()) {
    if (![10, 30, 60].includes(minutes))
        throw new GameError("BAD_DURATION", "Thời gian thám hiểm không hợp lệ.");
    const character = await db.character.findUniqueOrThrow({ where: { id: characterId } });
    if (!character.locationId)
        throw new GameError("NO_LOCATION", "Bạn chưa có địa điểm.");
    const active = await db.explorationActivity.findFirst({ where: { characterId, status: ActivityStatus.ACTIVE } });
    if (active)
        throw new GameError("ACTIVE_ACTIVITY", "Bạn đang thám hiểm.");
...
export async function claimExploration(db, characterId, activityId, now = new Date()) {
    return db.$transaction(async (tx) => {
        const job = await tx.explorationActivity.findUnique({ where: { id: activityId } });
        if (!job || job.characterId !== characterId)
            throw new GameError("NOT_FOUND", "Không tìm thấy chuyến thám hiểm.");
        if (job.status === ActivityStatus.CLAIMED)
            throw new GameError("ALREADY_CLAIMED", "Phần thưởng đã được nhận.");
        if (job.endsAt > now)
            throw new GameError("NOT_READY", "Chuyến thám hiểm chưa hoàn thành.");
        ...
        await tx.explorationActivity.update({ where: { id: activityId }, data: { status: ActivityStatus.CLAIMED, claimedAt: now, eventKey: "found-resource", reward: { item: roll.key, quantity: 1 } } });
...
export async function startTravel(db, characterId, routeId, now = new Date()) {
    return db.$transaction(async (tx) => {
        ...
        if (activeTravel)
            throw new GameError("ACTIVE_TRAVEL", "Bạn đang di chuyển.");
        const route = await tx.route.findUnique({
            where: { id: routeId },
            include: { origin: { include: { zone: true } }, destination: { include: { zone: true } } }
        });
        if (!route || !route.active)
            throw new GameError("ROUTE_NOT_FOUND", "Tuyến đường không khả dụng.");
        if (route.minimumRealmOrder > character.realmStage.realm.order)
            throw new GameError("REALM_REQUIREMENT_NOT_MET", "Bạn chưa đủ cảnh giới để đi tuyến này.");
        if (character.currentLocationId) {
            if (character.currentLocationId !== route.originId)
        ...
        const travel = await tx.travel.create({
            data: {
                ...
                encounterSnapshot: route.encounterTable
            }
        });
...
export async function claimTravel(db, characterId, travelId, now = new Date()) {
    return db.$transaction(async (tx) => {
        const travel = await tx.travel.findUnique({ where: { id: travelId }, include: { route: { include: { destination: true } } } });
        if (!travel || travel.characterId !== characterId)
            throw new GameError("NOT_FOUND", "Không tìm thấy chuyến đi.");
        if (travel.status === ActivityStatus.CLAIMED)
            throw new GameError("ALREADY_CLAIMED", "Chuyến đi đã được hoàn tất.");
        if (travel.endsAt > now)
            throw new GameError("NOT_READY", "Bạn chưa tới nơi.");
        ...
        const updated = await tx.travel.updateMany({
            ...
            data: { status: ActivityStatus.CLAIMED, claimedAt: now, encounterKey: encounter.key, encounterResult: encounterResult }
        });
        if (updated.count !== 1)
            throw new GameError("ALREADY_CLAIMED", "Chuyến đi đã được hoàn tất.");
...
async function resolveTravelEncounter(tx, characterId, encounterKey, dangerLevel) {
    if (encounterKey === "resource-cache") {
        ...
        if (!template)
            return { kind: encounterKey, message: "Bạn phát hiện dấu vết tài nguyên nhưng không thu được gì." };
        await tx.itemInstance.create({ data: { ownerId: characterId, templateId: template.id, quantity: 1 } });
    ...
    if (encounterKey === "wandering-monster") {
        ...
        if (!monster)
            return { kind: encounterKey, message: "Yêu khí thoáng qua rồi tan biến." };
        const result = simulateCombat({ name: character.name, hp: character.hp, attack: character.attack, defense: character.defense, speed: character.speed }, { name: monster.name, hp: monster.hp + dangerLevel * 8, attack: monster.attack + dangerLevel * 2, defense: monster.defense + dangerLevel, speed: monster.speed }, seededRng(seedFromString(`${characterId}:${dangerLevel}`)));
...
export async function fightMonster(db, characterId, monsterKey, seed = Date.now()) {
    return db.$transaction(async (tx) => {
        ...
        const result = simulateCombat({ name: character.name, hp: character.hp, attack: character.attack, defense: character.defense, speed: character.speed }, { name: monster.name, hp: monster.hp, attack: monster.attack, defense: monster.defense, speed: monster.speed }, seededRng(seed));
        ...
        if (reward.linhThach)
            await creditWallet(tx, characterId, Currency.LINH_THACH, BigInt(reward.linhThach), WalletTxType.REWARD, "Monster", monsterKey);
...
export async function purchaseMarketListing(db, buyerId, listingId, now = new Date()) {
    return db.$transaction(async (tx) => {
        const listing = await tx.marketListing.findUnique({ where: { id: listingId }, include: { item: true } });
        if (!listing || listing.status !== ListingStatus.ACTIVE || listing.expiresAt < now)
            throw new GameError("LISTING_INACTIVE", "Vật phẩm đã được người khác mua.");
        if (listing.sellerId === buyerId)
            throw new GameError("SELF_BUY", "Không thể mua vật phẩm của chính mình.");
